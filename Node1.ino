...
//  ENCRYPTION (DEMO ONLY) 
String encryptMessage(String input) {
  const char key = 0x5A;   // XOR demo key
  String encrypted = "";
...
String buildPage() {
  String page =
    ...
    "<title>CampusConnect - Node " + String(NODE_ID) + "</title>"
    ...
    "</head><body>"
      "<div class='wrapper'>"
        "<div class='card'>"
          ...
          "<div class='badge'>Node " + String(NODE_ID) + "</div>"
          ...
          "<form action='/send'>"
            ...
            "<select name='to'>"
              "<option value='0'>📡 Broadcast (All Nodes)</option>"
...
//  SEND (with encryption preview) 
void handleSend() {
  String msg = server.arg("msg");
  int toNode = server.arg("to").toInt();
  String senderName = server.arg("sender");

  // Encrypt ONLY for Serial monitor (not transmitted)
  String encryptedMsg = encryptMessage(msg);
  String hexView = toHex(encryptedMsg);

  Serial.println("----- Encryption Process -----");
  Serial.println("Original Message: " + msg);
  Serial.println("Encrypted (HEX): " + hexView);
  Serial.println("--------------");

  // Plain text to transmit
  String finalMsg = "From " + senderName + ": " + msg;

  if (toNode == 0) {
    int targets[] = {2, 3};
    for (int i = 0; i < 2; i++) {
      LoRa.beginPacket();
      LoRa.write((uint8_t)targets[i]);
      LoRa.write((uint8_t)NODE_ID);
      LoRa.print(finalMsg);       // ✅ Plain message sent
      LoRa.endPacket();
      delay(10);
    }
    statusMessage = "📡 Broadcast sent to all nodes";
    showOnDisplay(NODE_ID, 0, msg);
  } 
  else {
    LoRa.beginPacket();
    ...
    statusMessage = "Message sent to Node " + String(toNode);
...
void showOnDisplay(uint8_t from, uint8_t to, String msg) {
  ...
  display.println("");
  display.print("From Node: "); display.println(from);

  display.print("To Node: ");
  if (to == 0) {
    display.println("ALL");
  } else {
    display.println(to);
